...
async def complete_pipeline(file: UploadFile = File(...)):
    """
    Complete pipeline in one endpoint:
    1. Upload CSV to MongoDB
    2. Preprocess data
    3. Run MapReduce operations
    ...
    try:
        ...
        # Generate collection name
        file_name = file.filename.rsplit('.', 1)[0]
        coll_name = f"dataset_{re.sub(r'[^a-z0-9_]', '_', file_name.lower())}"
        
        # Parse CSV into documents
        documents = []
        for row in csv_reader:
            doc = {}
            for key, value in row.items():
        ...
        logger.info("💾 Step 2: Uploading to MongoDB...")
        collection = db[coll_name]
        result = collection.insert_many(documents)
        
        logger.info(f"✅ Inserted {len(result.inserted_ids):,} documents into {coll_name}")
        
        # STEP 3: Preprocess (remove nulls and duplicates)
        logger.info("🔧 Step 3: Preprocessing...")
        initial_count = len(result.inserted_ids)
        
        # Remove documents with all null values
        collection.delete_many({
            '$nor': [
                {key: {'$exists': True, '$ne': None, '$ne': ''}} 
                for key in collection.find_one().keys()
            ]
        })
        
        after_clean = collection.count_documents({})
        removed = initial_count - after_clean
        
        logger.info(f"✅ Removed {removed} invalid records. {after_clean:,} records remain")
        ...
        temp_field = next((f for f in ['AverageTemperature', 'temperature', 'temp', 'avg_temp'] if f in fields), None)
        group_field = next((f for f in ['Country', 'City', 'State'] if f in fields), 'category')
        date_field = next((f for f in ['dt', 'date', 'Date'] if f in fields), None)
        
        # Operation 1: Average by group
        if temp_field and group_field:
            try:
                result1 = list(collection.aggregate([
                    {'$match': {temp_field: {'$exists': True, '$type': 'double'}}},
                    {'$group': {
                        '_id': f'${group_field}',
                        'avg': {'$avg': f'${temp_field}'},
                        'min': {'$min': f'${temp_field}'},
                ...
                mapreduce_results['avg_by_group'] = [convert_to_serializable(r) for r in result1]
                operations.append('avg_by_group')
                logger.info(f"✅ Operation 1: {len(result1)} groups analyzed")
            except Exception as e:
                logger.warning(f"⚠️ Operation 1 failed: {e}")
        
        # Operation 2: Records by date
        if date_field:
            try:
                ...
                mapreduce_results['by_date'] = [convert_to_serializable(r) for r in result2]
                operations.append('by_date')
                logger.info(f"✅ Operation 2: {len(result2)} time periods analyzed")
            except Exception as e:
                logger.warning(f"⚠️ Operation 2 failed: {e}")
        ...
        if temp_field:
            try:
                ...
                mapreduce_results['stats'] = [convert_to_serializable(r) for r in result3]
                operations.append('stats')
                logger.info(f"✅ Operation 3: Statistics calculated")
            except Exception as e:
                logger.warning(f"⚠️ Operation 3 failed: {e}")
        
        # Operation 4: Distribution
        if group_field:
            try:
                ...
                mapreduce_results['distribution'] = [convert_to_serializable(r) for r in result4]
                operations.append('distribution')
                logger.info(f"✅ Operation 4: Distribution analyzed")
            except Exception as e:
                logger.warning(f"⚠️ Operation 4 failed: {e}")
        
        # Get sample data
        sample_data = list(collection.find().limit(5))
        sample_data = [convert_to_serializable(d) for d in sample_data]
        ...
        return {
            ...
            'message': f'✅ Pipeline complete! {after_clean:,} records processed, {len(operations)} analyses completed'
        }
